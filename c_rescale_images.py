...
def calculate_background(filename,
    ...
    with h5py.File(img_h5f_name,"w") as h5f:
        for k in modes:
            ds_key=f"rescaled_image_{k}"
            h5f.create_dataset(ds_key,shape=(sizeS,sizeT,sizeC,sizeZ,sizeY,sizeX),dtype=np.float32)
            h5f[ds_key].attrs["dimension_order"]="stczyx"
            h5f[ds_key].attrs["channels"]=np.array([c["@Fluor"] for c in channels],dtype="S")
        for _, row in tqdm(list(planes_df.iterrows())):
            ...
            for k in modes:
                ds_key=f"rescaled_image_{k}"
                if k=="divide":
                    h5f[ds_key][s,t,c,z,:,:]=(img-camera_dark)/backgroundss[c]
                elif k=="subtract":
                    h5f[ds_key][s,t,c,z,:,:]=(img-camera_dark)-backgroundss[c]
                else:
                    h5f[ds_key][s,t,c,z,:,:]=img-camera_dark
...
#            print(c_indices)
            ...
            first_img=np.array(h5f["rescaled_image_divide"][0,0,c_index,0,:,:])
            ...
            for (t,z),grp in tqdm(list(selected_planes_df.groupby(["T_index","Z_index"]))):
                ...
                with open(configuration_csv_path,"w") as fc:
                    ...
                    for j, row in grp.iterrows():
                        c,t,z,s=int(row["C_index"]),int(row["T_index"]),int(row["Z_index"]),int(row["image"]) 
                        img=np.array(h5f["rescaled_image_divide"][s,t,c,z,:,:])
