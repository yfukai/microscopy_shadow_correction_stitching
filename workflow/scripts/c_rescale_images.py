...
def rescale_images(
    ...
    *,
    ...
    background_key = (
        f"{'smoothed' if background_smoothing else 'rescaled'}_"
        + f"{background_method}"
    )
    params_dict["background_key"] = background_key
    backgroundss = {}
    for iC, iZ in itertools.product(range(sizeC), range(sizeZ)):
        bg_image_path = path.join(
            bg_directory, f"{background_key}_C{iC}_{channel_names[iC]}_Z{iZ}.tiff"
        )
        ...
        while np.any(background <= 0):
            ...
            for i, j in zip(*np.where(background <= 0)):
                background[i, j] = np.median(
                    background[
                        trunc_range(i, background.shape[0]),
    ...
    planes_df["row_col_label"] = planes_df.apply(
        lambda row: f'row{int(row["Y_index"])+1:03d}_col{int(row["X_index"])+1:03d}',
        axis=1,
    )
        ...
        for s, grp in tqdm(list(planes_df.groupby("image"))):
            ...
            for _, row in grp.iterrows():
                ...
                if c in nonuniform_background_subtract_c_indices:
                    img_small = transform.rescale(
                        img,
                        nonuniform_background_shrink_factor,
                        preserve_range=True,
                    )
                    img_small_bg = filters.median(
                        img_small, disk(nonuniform_background_median_disk_size)
                    )
                    img_bg = transform.resize(
                        img_small_bg, img.shape, preserve_range=True
                    )
                    img = img - img_bg
