...
def summarize_image(row, reader, thumbnail_size, quantile):
    ...
    hq = np.quantile(img.flatten(), 1.0 - quantile)
    img_trunc = img[np.logical_and(img > lq, img < hq)].flatten()
    thumb = transform.rescale(img, thumbnail_size / img.shape[0], preserve_range=True)
    return (
        thumb,
        np.max(img_trunc),
        np.min(img_trunc),
...
def calculate_background(
    ...
    *,
    ...
    if check_validity_channel:
        ...
        res = bview.map_async(
            _summarize_image, [row for _, row in list(planes_df.iterrows())]
        )
        ...
        for iC, grp in planes_df.groupby("C_index"):
            ...
            h, *edges, im = ax.hist2d(
                grp["mean"], grp["stdev"], bins=intensity_bin_size
            )
        ...
        for iS, grp in ph_planes_df.groupby("S_index"):
            ...
            savefig(
                fig,
                path.join(thumbail_output_name, f"2_thresholded_thumbnails_{iS}.pdf"),
            )
            plt.close("all")
        ...
        print("ok")
        ph_planes_df.drop("thumbnail", axis=1).to_csv(
            path.join(log_dir, "ph_planes_df.csv")
        )
        ...
        fig, axes = plt.subplots(1, 2, figsize=(10, 5))
        im = axes[0].scatter(
            series_df["X"], series_df["Y"], c=series_df["is_valid_ratio"]
        )
