...
def rescale_background(
    ...
    *,
    ...
    camera_propss = [
        get_camera_props(meta, channel, wf_match_keys=match_keys)
    ...
    if camera_dark_path:
        ...
        for channel, (camera_props, boundary) in zip(channels, camera_propss):
            ...
            camera_dark_img.append(
                np.zeros((get_size(boundary[1], sizeY), get_size(boundary[0], sizeX)))
            )
    ...
    for iC, iZ, img_key in itertools.product(
        range(sizeC), range(sizeZ), ["median", "mean"]
    ):
        ...
        ims = []
        imgs = [camera_dark_img[iC], bg_img, rescaled, rescaled_smoothed]
        ...
        for j, (img, name) in enumerate(zip(imgs, names)):
            if j > 1:
                io.imsave(
                    path.join(
                        rescaled_bg_directory, f"{name}_{path.basename(image_path)}"
                    ),
                    img,
                )
    ...
    print(params_dict)
    params_path = path.join(rescaled_bg_directory, "rescale_background_params.yaml")
