...
def process_stitching(
    ...
    *,
    ...
    fig.savefig(
        path.join(stitching_log_directory, "stitching1_row_and_col_position.pdf")
    )

    fig, ax = plt.subplots(1, 1)
    ax.plot(stitching_df2["x_pos2"] - stitching_df2["X"], label="x")
    ax.plot(stitching_df2["y_pos2"] - stitching_df2["Y"], label="y")
    ax.set_xlabel("series")
    ax.set_ylabel("difference b/w original and assigned position (px)")
    ax.legend()
    fig.savefig(
        path.join(stitching_log_directory, "stitching2_position_difference.pdf")
    )

    fig, ax = plt.subplots(1, 1)
    ax.plot(
        stitching_df2["col"],
        stitching_df2["x_pos2"] - stitching_df2["X"],
        ".",
        label="x",
    )
    ax.plot(
        stitching_df2["row"],
        stitching_df2["y_pos2"] - stitching_df2["Y"],
        ".",
        label="y",
    )
    ax.set_xlabel("series")
    ...
    ax.legend()
    fig.savefig(
        path.join(stitching_log_directory, "stitching3_deviation_per_row_col.pdf")
    )

    for rescale_method in rescale_methods:
        rescaled_image_key = f"rescaled_image_{rescale_method}"
        to_input_zarr_path = lambda row: path.join(
            rescaled_image_directory,
            rescaled_image_key,
            f"S{row['S_index']:03d}_{row['row_col_label']}.zarr",
        )
        stitching_df2["input_zarr_path"] = stitching_df2.apply(
            to_input_zarr_path, axis=1
        )
